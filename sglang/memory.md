...
│  │ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ││
│  │ req_to_token: (num_req, max_ctx_len) → slot_id                  ││
│  │ dtype: int32  |  size: 小（仅索引）                              ││
│  │ 所有层共享同一映射表                                              ││
│  │                                                                   ││
│  │ 例: req_to_token[5, 0:100] = [1000, 1001, ..., 1099]            ││
│  │     ↑ Request 5 的前 100 个 token 映射到 slot 1000-1099         ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                    ↓ slot_id 索引                     │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ 第二级：KVCache（物理存储）                                       ││
...
│  │                                                                   ││
│  │ ┌─ MHA 路径（默认）──────────────────────────────────────────┐  ││
│  │ │ k_buffer: List[Tensor]  (layer_num 个独立 tensor)          │  ││
│  │ │ k_buffer[layer_id].shape = (size, head_num, head_dim)       │  ││
│  │ │                             ↑      ↑         ↑               │  ││
│  │ │                          slot_id  head    dimension          │  ││
│  │ │                                                               │  ││
│  │ │ 例: size=65536, head_num=32, head_dim=128                    │  ││
│  │ │     65536 × 32 × 128 × 2B = 536,870,912B ≈ 0.5 GiB(/层）      │  ││
│  │ │                                                               │  ││
│  │ │ 特性:                                                         │  ││
│  │ │ • 同一 slot_id 跨层对应同一 token（逻辑对齐）               │  ││
│  │ │ • 但每层物理独立（非 4D tensor，是 List）                   │  ││
│  │ │ • Layout: layer-first                                        │  ││
│  │ └─────────────────────────────────────────────────────────────┘  ││
│  │                                                                   ││
│  │ ┌─ MLA 路径（低秩压缩）──────────────────────────────────────┐  ││
│  │ │ kv_buffer[layer_id].shape = (size, 1, kv_cache_dim)         │  ││
│  │ │                              ↑     ↑   ↑                     │  ││
│  │ │                           slot   合并  lora_rank+rope_dim    │  ││
│  │ │                                                               │  ││
│  │ │ 例: kv_cache_dim = 512 + 64 = 576                            │  ││
│  │ │     不区分 head，低秩表示                                     │  ││
│  │ │     NSA: 可能用 FP8 量化                                      │  ││
│  │ └─────────────────────────────────────────────────────────────┘  ││
│  │                                                                   ││
│  │ ┌─ SWA 路径（滑动窗口）──────────────────────────────────────┐  ││
│  │ │ full_kv_pool: 全注意力层的 KV                               │  ││
│  │ │ swa_kv_pool:  窗口层的 KV（容量更小）                        │  ││
│  │ │ full_to_swa_index_mapping: slot 转换映射                     │  ││
│  │ └─────────────────────────────────────────────────────────────┘  ││
│  │                                                                   ││
│  │ ┌─ DSA 路径（双稀疏）────────────────────────────────────────┐  ││
│  │ │ + label_buffer: 额外的标签缓存                              │  ││
│  │ │   KV cache 大小不变，增加 label 辅助                        │  ││
│  │ └─────────────────────────────────────────────────────────────┘  ││
│  │                                                                   ││
│  │ dtype: float16/bfloat16/float8  |  size: 巨大（实际向量）       ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                       │
│  ┌─ Allocator（内存管理）────────────────────────────────────────┐  │
│  │ • TokenToKVPoolAllocator (page_size=1, token-level)            │  │
│  │ • PagedTokenToKVPoolAllocator (page_size>1)                    │  │
│  │ • SWATokenToKVPoolAllocator (双池管理)                         │  │
│  │                                                                 │  │
│  │ API: alloc(n) → slot_ids,  free(slot_ids)                      │  │
│  │      merge_and_sort_free() ← PD 分离时调用                     │  │
│  └─────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
                  ↕ transfer_kv_* kernels (CUDA)
┌─────────────────────────────────────────────────────────────────────┐
│  Host 端（CPU RAM）- HiCache 第三级（可选）                          │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ HostKVCache                                                      ││
│  │ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━   ││
│  │                                                                   ││
│  │ Layout 选项:                                                      ││
│  │ ┌─ layer_first ─────────────────────────────────────────────┐   ││
│  │ │ (kv, layer, size, head, dim)  ← 与 GPU 同结构              │   ││
│  │ │ 适合: 逐层传输                                              │   ││
│  │ └─────────────────────────────────────────────────────────────┘   ││
│  │ ┌─ page_first ──────────────────────────────────────────────┐   ││
│  │ │ (kv, size, layer, head, dim)  ← 优化 I/O                   │   ││
│  │ │ 适合: Mooncake/外部存储，以 page 为单位批量传输            │   ││
│  │ └─────────────────────────────────────────────────────────────┘   ││
│  │ ┌─ page_first_direct ───────────────────────────────────────┐   ││
│  │ │ 直接 I/O，跳过 kernel                                       │   ││
│  │ └─────────────────────────────────────────────────────────────┘   ││
│  │                                                                   ││
│  │ Transfer Kernels:                                                 ││
│  │ • transfer_kv_per_layer_pf_lf: page_first → layer_first          ││
│  │ • transfer_kv_all_layer_lf_pf: layer_first → page_first          ││
│  │ • transfer_kv_per_layer_mla_*: MLA 专用                           ││
│  │                                                                   ││
│  │ 触发: --enable-hierarchical-cache                                 ││
│  └─────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
                  ↕ Storage Backend (Mooncake/HF3FS/EIC)
┌─────────────────────────────────────────────────────────────────────┐
│  External Storage（可选第四级）                                       │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │
│  • Mooncake: 分布式 KV cache，要求 page_first                        │
│  • HF3FS: Hugging Face 3FS 存储                                      │
│  • EIC: 火山引擎云存储                                                │
│                                                                       │
│  用途: 超长上下文（百万 token），GPU+Host 都放不下                   │
└─────────────────────────────────────────────────────────────────────┘

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
KVPress 集成点（在 GPU 第二级）:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Attention Backend (flashattention_backend.py)
      ↓ 生成 K, V
  【插入点】compress(keys, values, loc, allocator)
      ├─ score → topk → gather (新内存)
      ├─ allocator.free(pruned_slots)  ← 关键！
      └─ 返回 compressed_keys, compressed_values, compressed_loc
      ↓
  set_kv_buffer(layer, compressed_loc, compressed_k, compressed_v)
      ↓ 写入 k_buffer[layer_id]
  
  约束:
  • 必须 Token-wise（所有 head 保留相同 token）
  • 逐层压缩（layer_id 独立）
  • 峰值内存 = 旧 KV + 新 KV (gather 不可避免)
  • 立即 free 避免多层累积
```
